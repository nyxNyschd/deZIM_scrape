...
async def take_screenshot(url, browser, page):
    ...
    #enter cookies-iframe
    frame=await page.waitForSelector('#sp_message_iframe_494009')
    frame_access = await frame.contentFrame()
    await frame_access.evaluate('''() =>
            {
               const accept_button = document.querySelector('body > div > div.message > button');
              accept_button.click();    
            }''')
    current_time = str(datetime.datetime.now())
    screenshot = await page.screenshot({'path': './zeit_' + current_time + '.jpg', 'type': 'jpeg', 'fullPage': True})
